...
async function init(){
  ...
  try {
    ...
    // Exactly one Supabase client, following the working Submission pattern.
    db = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
    db.auth.onAuthStateChange((event, session) => {
      if (session?.user?.id) {
        user = session.user;
        console.info("[QC] Auth state:", event);
      }
    });
    ...
    user = session.user;
    console.info("[QC] Supabase client: OK");
    console.info("[QC] Session: OK", user.id);

    bindShell();
    await loadIdentity();
    console.info("[QC] User profile: OK");

    await resolveProject();
    console.info("[QC] Project: OK", project?.id);

    await loadData();
    console.info("[QC] Quality data: OK");

    renderAll();
    finishLoading();
  } catch (e) {
    console.error("❌ Quality Alerts initialization error:", e);
    finishLoading();
    const box = document.getElementById("qcInitError");
    if (box) {
      box.textContent = "Quality Alerts could not load: " + (e?.message || "Unknown error");
...
async function loadIdentity(){
 const {data,error}=await db.from("user_profiles").select("first_name,email").eq("id",user.id).maybeSingle();
 if(error) throw new Error("user_profiles: " + (error.message || "request failed"));
...
async function resolveProject(){
 ...
 if(error) throw new Error("project_members: " + (error.message || "request failed"));
 if(!data?.length) throw Error("No active project membership found.");
