...
async function loadUserAndProjects() {
    ...
    try {
        ...
        // Profile is optional. The page can still operate using auth email if
        // user_profiles is unavailable or temporarily blocked by RLS.
        let profile = null;
        ...
        if (profileResult.error) {
            console.warn('⚠️ user_profiles could not be loaded:', profileResult.error);
        } else {
        ...
        document.getElementById('userAvatar').textContent = (firstName.charAt(0) || currentUser.email?.charAt(0) || 'U').toUpperCase();
        ...
        if (!memberships.length) {
            document.getElementById('userRole').textContent = 'NO PROJECT';
            showNotification('No active project membership was found for this account.', 'warning');
            return;
        }

        allUserProjects = memberships.filter(m => m.projects);
        const primaryRole = memberships[0]?.role || 'viewer';
        document.getElementById('userRole').textContent = primaryRole.replace(/_/g, ' ').toUpperCase();

        // Prevent duplicate role badges after refresh/reinitialization.
        document.querySelectorAll('.user-info .role-badge').forEach(el => el.remove());
        const roleBadge = document.createElement('span');
        roleBadge.className = `role-badge ${primaryRole}`;
        roleBadge.textContent = primaryRole.toUpperCase();
        const syncBtn = document.querySelector('.sync-btn');
        if (syncBtn) document.querySelector('.user-info').insertBefore(roleBadge, syncBtn);
        ...
        if (projectIdFromUrl && projectIdFromUrl !== 'all') {
            targetProject = allUserProjects.find(m => m.projects?.id === projectIdFromUrl);
        }

        if (!targetProject) {
            const lastViewed = localStorage.getItem(`lastProject_${currentUser.id}`);
            if (lastViewed) targetProject = allUserProjects.find(m => m.projects?.id === lastViewed);
            if (!targetProject) targetProject = allUserProjects[0];
        }

        const isOwner = memberships.some(m => m.role === 'owner');
        if (isOwner && allUserProjects.length > 1) {
            document.getElementById('projectSelectorContainer').classList.remove('hidden');
            await populateDropdown(allUserProjects);
        }

        if (!targetProject?.projects) {
            throw new Error('No valid project is available for this account.');
        }
...
function getSupabaseErrorMessage(error) {
    if (!error) return 'Quality Alerts could not be initialized.';
    const message = error.message || error.error_description || String(error);
    if (/invalid api key/i.test(message)) {
        return 'Supabase rejected the API key. Use the same current Supabase publishable/anon key as the working Submission page.';
    }
    if (/401|unauthorized/i.test(message)) {
        return 'Supabase returned 401 Unauthorized. Check the API key and authenticated session.';
    }
    return message;
}
